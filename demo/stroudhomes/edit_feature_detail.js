function edit_feature(verbose_name, tableId, list_pointer, base_id_col, id_col, id_val, field_list){
	...
	$edit_nav = $nav_ul.children("li[id='edit-tab-nav']");
	$edit_nav.addClass("active");

	var pane_id;
	pane_id = $edit_nav.children("a").attr('href');
	$nav_pane = $nav.children(".tab-content");
	$nav_pane.children("div").removeClass("active");
	$edit_pane = $nav_pane.children(pane_id);
	$edit_pane.addClass("active");
	$edit_fields = $edit_pane.find("#edit_fields");

	$edit_fields.empty();
	$submit = $nav_pane.find("button[id='edit_submit']");
	for (i=0; i< field_list.length; i++){
		$edit_fields.append($format_row(field_list[i]));
	}
	var input_id_string = '<input type="hidden" class="id_col" value="{ID_VALUE}" name="{ID_COL}">'.replace(/{ID_COL}/g, base_id_col).replace(/{ID_VALUE}/g, id_val);
	...
	$submit.on("click", function(){
		...
		$input_fields = $edit_fields.find("input").not($id_field);
